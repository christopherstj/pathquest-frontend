...
export const GET = async (req: NextRequest) => {
    ...
    const token = await getSessionToken().catch((err) => {
        console.error("[dashboard-stats] Failed to get Google ID token:", err);
        return null;
    });
    ...
    try {
        const data = await client.fetchJson("/dashboard/stats");
        return NextResponse.json(data, { status: 200 });
    } catch (err: any) {
        console.error("Error fetching dashboard stats:", err?.bodyText ?? err);
        return NextResponse.json(
