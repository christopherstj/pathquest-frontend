...
type PublicSummitCardProps = {
    summit: PublicSummitCardSummit;
    showPeakHeader?: boolean;
};

export const PublicSummitCard = ({
    summit,
        ...
        <div className="p-4 rounded-xl bg-card border border-border/70">
            {showPeakHeader && summit.peak_id && summit.peak_name && (
                <div className="mb-3 pb-3 border-b border-border/50">
                    <Link
                        href={`/peaks/${summit.peak_id}`}
                        className="flex items-center gap-2 text-sm font-medium text-foreground hover:text-primary transition-colors"
                    >
                        <Mountain className="w-4 h-4 text-secondary" />
                        <span className="truncate">{summit.peak_name}</span>
                    </Link>
                </div>
            )}
                ...
                <div className="flex flex-wrap gap-2 mt-3">
                    {summit.difficulty && (
                        <div
                            ...
                            } ${DIFFICULTY_CONFIG[summit.difficulty].borderColor} ${
                    ...
                    {summit.experience_rating && (
                        <div
                            className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full border text-xs ${
                                ...
                                EXPERIENCE_CONFIG[summit.experience_rating]
                                    .borderColor
                            } ${
                                EXPERIENCE_CONFIG[summit.experience_rating]
                                    .bgColor
            ...
            {((summit.condition_tags && summit.condition_tags.length > 0) ||
                (summit.custom_condition_tags &&
                    summit.custom_condition_tags.length > 0)) && (
                    ...
                    <div className="grid grid-cols-2 gap-2">
                        {summit.temperature !== undefined && (
                            <div className="flex items-center gap-1.5">
                                ...
                                <span className="text-sm text-foreground">
                                    {Math.round(
                                        celsiusToFahrenheit(summit.temperature)
                                    )}
                                    °F
                                </span>
                            </div>
                        ...
                        {summit.wind_speed !== undefined && (
                            <div className="flex items-center gap-1.5">
                                ...
                                <span className="text-sm text-foreground">
                                    {Math.round(kmhToMph(summit.wind_speed))}{" "}
                                    mph
                                </span>
                            </div>
                        )}
...
export default PublicSummitCard;
