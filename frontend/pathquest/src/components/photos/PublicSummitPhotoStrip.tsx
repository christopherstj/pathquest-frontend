...
import getPublicSummitPhotos from "@/actions/photos/getPublicSummitPhotos";
import type { SummitType, PublicSummitPhoto } from "@pathquest/shared/types";

type PublicSummitPhotoStripProps = {
    summitId: string;
...
/**
 * A horizontal strip of photo thumbnails for a public summit.
 * Used in the community section to show photos on public summit cards.
 * Clicking a thumbnail opens a lightbox with navigation.
 */
const PublicSummitPhotoStrip = ({
    summitId,
    summitType,
    maxThumbnails = 4,
}: PublicSummitPhotoStripProps) => {
    const [lightboxIndex, setLightboxIndex] = useState<number | null>(null);

    const { data: photosResult, isLoading } = useQuery({
        queryKey: ["publicSummitPhotos", summitType, summitId],
        queryFn: () => getPublicSummitPhotos({ summitType, summitId, limit: 10 }),
        enabled: !!summitId,
...
export default PublicSummitPhotoStrip;
